...
public final class Util
{
	...
	public enum LOG {INFO, WARNING, ERROR, EXCEPTION, DEBUG};
...
//		return (int)Math.round((double)(length*dstDpi)/(double)srcDpi);
	 ...
	 * @return 選択したファイル、失敗した場合、キャンセルした場合はnull
	 */
	public static File selectFile(String fileName, String defFileName, FileNameExtensionFilter[] extList, boolean toSave)
	{
		Util.initDlg();
		...
		if (toSave)
		{
			...
			if (file.exists())
			{
				...
				if (JOptionPane.NO_OPTION == JOptionPane.showConfirmDialog(null, message, "確認", JOptionPane.YES_NO_OPTION))
				{
					return null;
				}
			}
	...
	public static void log(LOG type, Exception ex, String format, Object...args)
	{
		String text = (null == args ? format: String.format(format, args));
		StackTraceElement[] trace = ex.getStackTrace();
		StringBuilder line = new StringBuilder(String.format("%s %s %s\nStack trace", text, ex.toString(), ex.getMessage()));
		for (int i = 0; i < trace.length; i++)
		{
			line.append("\n\t");
			line.append(trace[i]);
	...
	private static int showSelectDialog(String title, int messageType, String[] selections, int defIndex, boolean catchMouse, String format, Object...args)
	{
		String message = (null == args ? format: String.format(format, args));
		JOptionPane pane = new JOptionPane(message, messageType, JOptionPane.DEFAULT_OPTION, null, selections, selections[defIndex]);
	...
	private static int showQuestionDialog(String title, int messageType, int optionType, String format, Object...args)
	{
		return showQuestionDialog(title, messageType, optionType, true, format, args);
	}

	private static int showQuestionDialog(String title, int messageType, int optionType, boolean catchMouse, String format, Object...args)
	{
		String message = (null == args ? format: String.format(format, args));
	...
	private static void showMessageDialog(String title, int messageType, boolean catchMouse, String format, Object...args)
	{
		String message = (null == args ? format: String.format(format, args));
	...
	{
		return (showQuestionDialog("選択", JOptionPane.QUESTION_MESSAGE, JOptionPane.YES_NO_OPTION, catchMouse, format, args) == JOptionPane.YES_OPTION);
	}
	...
	{
		return showQuestionDialog("選択", JOptionPane.QUESTION_MESSAGE, JOptionPane.YES_NO_CANCEL_OPTION, catchMouse, format, args);
	}
	 ...
	 * @return 選択結果
	 */
	public static int select3(String title, String[] selections, int defIndex, boolean catchMouse, String format, Object...args)
	{
	...
	{
		return (showQuestionDialog("確認", JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION, format, args) == JOptionPane.OK_OPTION);
	}
